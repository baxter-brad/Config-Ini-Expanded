...
# Note: the labels for the non-loop LVAR's below (e.g., "name") may
#       be the same in different loops, but the names of the loops
#       must be different than their parents's names, e.g.,
#       "color_sales" is different that "period_sales", which differs
#       from "sales" (the top level loop name).  This is because we
#       plan to nest like this:
...
# ... and they wouldn't parse correctly if they all said "sales".
# All in all, forcing different names in nested loops is a Good
# Thing anyway.

# Note: the outer "loop" has a single element so we can get the
#       columns defined at a higher level.  This is a bit contrived,
#       but lets us also test some complex nesting.
...
cmt  = Spreadsheet (qualified as needed)
tmpl = <<:chomp
{LOOP:sales}Spreadsheet:{LOOP:periods}{LOOP:period_sales}{IF_LC:first}
         {LOOP:columns} {LVAR:name}{END_LOOP:columns}
{LVAR:periods:name}{ELSE}   {END_IF_LC:first} {LVAR:name}{LOOP:color_sales} {LVAR:sales}{END_LOOP:color_sales}
